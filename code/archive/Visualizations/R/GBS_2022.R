...
iNatUY <- read_csv('Visualizations/data/observations-278073.csv', guess_max = 1000)
...
iNatUY %>% 
  filter(created_at>=(ymd(today())-years(1))) %>% nrow()

# Percentage of the records created a year from today over the total amount of records
(100 * iNatUY %>% 
  filter(created_at>=(ymd(today())-years(1))) %>% nrow())/nrow(iNatUY) 

# Number of users a year from today
iNatUY %>% 
  filter(created_at>=(ymd(today())-years(1))) %>% 
...
spatialPlot <- left_join(Uruguay %>% mutate(place_admin1_name=tolower(nombre)), 
          iNatUY %>% 
            group_by(place_admin1_name) %>% count() %>% 
            mutate(place_admin1_name=stri_trans_general(tolower(place_admin1_name), id = 'Latin-ASCII')), 
          by='place_admin1_name') %>% 
  ggplot() + 
  geom_sf(aes(fill=n)) +
  labs(x='', y= '', fill = 'Número de\n Observaciones') +
  theme_bw() +
  scale_fill_gradientn(colours = wesanderson::wes_palette("Zissou1", 10, type = "continuous")) +
  theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))) 

# ggsave(plot = spatialPlot, filename='figs/GBS_2022_spatialPlot.svg', device = 'svg', width=6, height=6, dpi=300)
...
taxonPlot <- iNatUY %>% 
  ...
  scale_fill_brewer(palette ='Spectral') +
  #theme(text=element_text(family='Calibri')) +
  theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))) + xlab('Número de observaciones')

# ggsave(plot = taxonPlot, filename='figs/GBS_2022_taxonPlot.svg', device = 'svg', width=8, height=6, dpi=300)
